...
// Methods
    ...
    public bool Compare( string sourceFile, string changedFile, bool bFragments, string ignoreFile = null ) 
    {
        return Compare( sourceFile, changedFile, bFragments, null, ignoreFile );
    }
    ...
    /// <returns>True, if the documents/fragments are identical.</returns>
    public bool Compare( string sourceFile, string changedFile, bool bFragments, XmlWriter diffgramWriter, string ignoreFile = null) 
    {
        if ( sourceFile == null )
        ...
        try
        {
            ...
            if ( bFragments )
                OpenFragments( sourceFile, changedFile, ignoreFile, ref sourceReader, ref targetReader );
            else
                OpenDocuments( sourceFile, changedFile, ignoreFile, ref sourceReader, ref targetReader );
    ...
    /// <summary>
    /// IMANOR 24-09-2013 added the ignoreFile concept
    /// </summary>
    /// <param name="sourceFile"></param>
    /// <param name="changedFile"></param>
    /// <param name="ignoreFile"></param>
    /// <param name="sourceReader"></param>
    /// <param name="changedReader"></param>
    private void OpenDocuments( String sourceFile, String changedFile, String ignoreFile,  
                                ref XmlReader sourceReader, ref XmlReader changedReader )
    {
        string ignoredSourceFile = null;
        string ignoredChangedFile = null;

        if (ignoreFile != null)
        {
            HandleIgnoredElements(sourceFile, changedFile, ignoreFile, ref ignoredSourceFile, ref ignoredChangedFile);
        }

        XmlTextReader tr = new XmlTextReader(ignoredSourceFile != null ? ignoredSourceFile : sourceFile);
        tr.XmlResolver = null;
        sourceReader = tr;

        tr = new XmlTextReader(ignoredChangedFile != null ? ignoredChangedFile : changedFile);
        tr.XmlResolver = null;
        changedReader = tr;
    }
    ...
    /// <summary>
    /// IMANOR 24-09-2013
    /// This method removes the elements we want to ignore. that way they are not being compared
    /// </summary>
    /// <param name="sourceFile"></param>
    /// <param name="changedFile"></param>
    private void HandleIgnoredElements(string sourceFile, string changedFile, string ignoreFile, ref string ignoredSourceFile, ref string ignoredChangedFile)
    {
        var ignoreElements = LoadIgnoreFile(ignoreFile);

        ignoredSourceFile = HandleIgnoredElementsForFile(sourceFile, ignoreElements);
        ignoredChangedFile = HandleIgnoredElementsForFile(changedFile, ignoreElements);
    }

    /// <summary>
    /// comments out ignored elements and saves the changed file as xxx_ignored.xml
    /// </summary>
    /// <param name="fileName"></param>
    /// <param name="ignoreElements"></param>
    /// <returns></returns>
    private string HandleIgnoredElementsForFile(string fileName, List<IgnoreElement> ignoreElements)
    {
        try
        {
            ...
            foreach (var ignore in ignoreElements)
            {
                ...
                if (ignore.ElementNamespaces.Count > 0)
                {
                    nsmgr = new XmlNamespaceManager(doc.NameTable);
                    foreach (var ns in ignore.ElementNamespaces)
                    {
                        nsmgr.AddNamespace(ns.Prefix, ns.Value);
                    }
                }
                XmlNodeList nodes = doc.SelectNodes(ignore.ElementXpath, nsmgr);
                for (int i = nodes.Count - 1; i >= 0; i--)
                {
                    if (conditionMet(nodes[i], ignore, nsmgr))
                    {
                        var content = nodes[i].OuterXml;
                        XmlNode newNode = doc.CreateComment(" [ignored] " + content);
                        nodes[i].ParentNode.ReplaceChild(newNode, nodes[i]);
                    }
                }                
            }
            var pos = fileName.LastIndexOf(".xml");
            var ignoredFileName = fileName.Insert(pos, "_ignored");
            doc.Save(ignoredFileName);
    ...
    private void OpenFragments(String sourceFile, String changedFile, String ignoreFile,  
                                ref XmlReader sourceReader, ref XmlReader changedReader )
    {
